...
import { Flame, Star, Sparkles, BookOpen, FolderOpen, Building2, Wallet, Package, Search, LayoutGrid, List } from 'lucide-react';
...
function PanelProducts({ viewMode, setViewMode }) {
    ...
    const getTitle = useMemo(() => {
        ...
        if (mode === 'promo') return wrap(Flame, 'Promo Hari Ini');
        if (mode === 'bestseller') return wrap(Star, 'Best Seller');
        if (mode === 'new') return wrap(Sparkles, 'Stok Terbaru');
        if (mode === 'az') return wrap(BookOpen, 'Semua Produk A–Z');
        if (categoryLabel) return wrap(FolderOpen, categoryLabel);
        if (filters.brand_slug) return wrap(Building2, filters.brand_slug.toUpperCase());
        if (filters.min_price !== null || filters.max_price !== null) {
            ...
            return wrap(Wallet, `Harga: ${min} - ${max}`);
        }
        if (filters.tags?.length) return wrap(Sparkles, filters.tags.join(', '));
        if (filters.category_slugs?.length) return wrap(FolderOpen, filters.category_slugs.join(', '));
        return wrap(Package, 'Produk tersedia');
    ...
    if (loading) {
        return (
            <div className="panel flex flex-col w-full">
                ...
                <div className="flex items-center justify-between flex-wrap mb-4">
                    ...
                    {!isMobile && (
                        <div className="flex gap-1">
                            <div className="h-7 w-12 bg-slate-200 rounded-md animate-pulse"></div>
                            <div className="h-7 w-12 bg-slate-200 rounded-md animate-pulse"></div>
                        </div>
                    )}
                </div>
                ...
                {/* Loading Message */}
                <div className="mb-3">
                    <p className="text-xs text-blue-600 animate-pulse">{loadingMessage}</p>
                </div>
