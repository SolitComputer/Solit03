...
import { Wrench, Clock, CheckCircle, Package, RefreshCw, Search, AlertCircle, Check, Hash, Radio } from "lucide-react";
  ...
  return (
    <div className="min-h-screen bg-surface-muted relative overflow-x-hidden">
      <Helmet>
        ...
        <meta
          name="description"
          content="Pantau status antrian servis laptop kamu di Solit 03 secara realtime — dari antrian, sedang dikerjakan, menunggu sparepart, hingga selesai."
        />
        <link rel="canonical" href="https://solit03.com/cek-antrian" />
      </Helmet>
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 py-10 md:py-16">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {STATUS_ORDER.map((s) => {
                ...
                return (
                  <div
                    key={s}
                    className={`rounded-xl p-4 border ${cfg.bg} ${cfg.border} transition-all hover:shadow-soft hover:-translate-y-0.5 duration-300`}
                  >
                        ...
                        <div className="divide-y divide-slate-50/80">
                          {items.map((order) => {
                            ...
                            return (
                              ...
                              >
                                {/* Nomor urut */}
                                <div className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 font-bold text-sm border shadow-soft-sm ${cfg.bg} ${cfg.text} ${cfg.border}`}>
                                  {order.no_urut}
                                </div>
                                  ...
                                  <div className="flex items-center gap-1.5 flex-wrap mb-0.5">
                                    ...
                                    {status === "DONE" && (
                                      <span className="inline-flex items-center gap-1 text-[10px] bg-emerald-100 text-emerald-700 px-1.5 py-0.5 rounded-full font-medium">
                                        <Check className="w-3 h-3" aria-hidden="true" /> Silakan diambil
                                      </span>
                                    )}
                                  </div>
        ...
        {!loading && filtered.length === 0 && !search && (
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 mt-12">
            {[
              { icon: Hash, title: "Temukan Nomor Urut", desc: "Nomor antrian diberikan saat laptop diterima oleh teknisi" },
              { icon: Radio, title: "Update Real-Time", desc: "Status berubah otomatis tanpa perlu refresh halaman" },
              { icon: Wrench, title: "Pantau Progress", desc: "Lihat antrian, dikerjakan, tunggu sparepart, atau selesai" },
            ].map((item, i) => (
        ...
        <div className="flex justify-center mt-12">
          <div className="flex gap-2">
            {[...Array(3)].map((_, i) => (
              <div
                key={i}
                className="w-1.5 h-1.5 rounded-full bg-blue-400 opacity-50 animate-pulse"
